...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        cours = list(FoadCour.objects.using('foad').filter(faculte__in=etape).values_list('code', flat=True))
        cours_user = FoadCourUser.objects.using('foad').filter(code_cours__in=cours, user__statut=5)
